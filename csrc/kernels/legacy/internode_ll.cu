...
__global__ __launch_bounds__(1024, 1) void dispatch_v2(void* packed_recv_x,
    ...
    if ((phases & LEGACY_LOW_LATENCY_SEND_PHASE) == 0)
        goto LOW_LATENCY_DISPATCH_V2_RECV;
            ...
            for (int i = thread_id; i < hidden_bf16_int4; i += num_threads) {
                ...
                if constexpr (kUseFP8) {
                    ...
                    if constexpr (kPackScaleWrites) {
                        ...
                        uint32_t b1 = __shfl_sync(0xffffffff, my_sf, group_leader + 2);
                        uint32_t b2 = __shfl_sync(0xffffffff, my_sf, group_leader + 4);
                        uint32_t b3 = __shfl_sync(0xffffffff, my_sf, group_leader + 6);
                        if (lane_id == group_leader) {
                            int rdma_x_scale_idx = i * kNumElemsPerRead / kNumPerChannels;
                            uint32_t packed = b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
                            reinterpret_cast<uint32_t*>(rdma_x_scales)[rdma_x_scale_idx / 4] = packed;
                        }
                    } else {
                        if (lane_id % 2 == 0) {
...
LOW_LATENCY_DISPATCH_V2_RECV:
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        const auto recv_x_scales = static_cast<scale_t*>(packed_recv_x_scales) +
            local_expert_idx * (kUseNVFP4 ? num_aligned_tokens : num_ranks * num_max_dispatch_tokens_per_rank) * num_aligned_scales;
        ...
        if (sub_warp_id == 1 and lane_id == 0) {
            ...
            if (num_recv_tokens == 0)
                num_recv_tokens = -1;
            if (wait_recv_cost > LEGACY_NUM_TIMEOUT_CYCLES) {
                printf("Warning: DeepEP timeout for dispatch_v2 receive, rank %d, local_expert_idx %d, src_rank %d\n",
                       rank,
                       local_expert_idx,
                       src_rank);
                if (mask_buffer_ptr == nullptr)
...
void dispatch_v2(void* packed_recv_x,
              void* packed_recv_x_scales,
              int* packed_recv_src_info,
              int64_t* packed_recv_layout_range,
              int* packed_recv_count,
...
#define DISPATCH_V2_LAUNCH_CASE(hidden) {                                              \
    ...
    SETUP_LAUNCH_CONFIG(num_sms, num_warps * 32, stream);
    SWITCH_HIDDEN(DISPATCH_V2_LAUNCH_CASE);
#undef DISPATCH_V2_LAUNCH_CASE
}
...
combine(void* combined_x,
    ...
    // Sending phase
    if ((phases & LEGACY_LOW_LATENCY_SEND_PHASE) == 0)
        goto LOW_LATENCY_COMBINE_V2_RECV;
