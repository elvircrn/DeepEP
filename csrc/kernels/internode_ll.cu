...
// Upstream DeepSeek combine kernel (no cooperative token partitioning, no src_signals)
template <bool kUseLogFMT, int kHidden, int kNumMaxTopk, int kNumMaxUnrolls>
__global__ __launch_bounds__(1024, 1) void
combine_upstream(void* combined_x,
        void* rdma_recv_x, int* rdma_recv_flag, void* rdma_send_x,
        const void* x, const int64_t* topk_idx, const float* topk_weights,
        const int* src_info, const int64_t* layout_range,
        int64_t* combine_wait_recv_cost_stats,
    ...
    // Sending phase
    if ((phases & LOW_LATENCY_SEND_PHASE) == 0)
        goto LOW_LATENCY_COMBINE_UPSTREAM_RECV;
    ...
    // Receiving phase
    LOW_LATENCY_COMBINE_UPSTREAM_RECV:
    if ((phases & LOW_LATENCY_RECV_PHASE) == 0)
        return;
...
auto combine_func = use_logfmt ? \
    combine_upstream<true, hidden, kNumMaxTopk, kNumMaxUnrolls> : \
    combine_upstream<false, hidden, kNumMaxTopk, kNumMaxUnrolls>; \
SET_SHARED_MEMORY_FOR_TMA(combine_func); \
LAUNCH_KERNEL(&cfg, combine_func, \
              combined_x, \
              rdma_recv_x, rdma_recv_flag, rdma_send_x, \
